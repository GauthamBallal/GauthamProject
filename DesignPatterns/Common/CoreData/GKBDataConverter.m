...
-(BOOL)checkIfQuestionIsAdded:(NSDictionary*)object
{
    ...
    pred = [NSPredicate predicateWithFormat:@"(questionID = %@) AND (testID = %@)",object[kCDQuestionID],object[kCDTestID]];
...
- (BOOL)exportFilesToCoreData
{
    ...
    for (NSDictionary *productDict in products) {
        if (![self checkIfQuestionIsAdded:productDict])
        {
            ...
            question.testID = [NSNumber numberWithInt:[[productDict objectForKey:kCDTestID] intValue]];
            question.testName = [productDict objectForKey:kCDTestName];
            question.testDescription = [productDict objectForKey:kCDTestDescription];
            question.question = [productDict objectForKey:kCDQuestion];
            question.questionID = [NSNumber numberWithInt:[[productDict objectForKey:kCDQuestionID] intValue]];
            question.optionOne = [NSString stringWithFormat:@"1. %@",[productDict objectForKey:kCDQuestionOption1]];
            question.optionTwo = [NSString stringWithFormat:@"2. %@",[productDict objectForKey:kCDQuestionOption2]];
            question.optionThree = [NSString stringWithFormat:@"3. %@",[productDict objectForKey:kCDQuestionOption3]];
            question.optionFour = [NSString stringWithFormat:@"4. %@",[productDict objectForKey:kCDQuestionOption4]];
            question.correctAnswer = [productDict objectForKey:kCDQuestionCorrectAnswer];
            question.hint = [productDict objectForKey:kCDQuestionHint];
...
- (NSArray *)parseTestsToArray:(NSString *)str
{
    ...
    for (NSArray *cols in rows) {
        ...
        for (int i=0 ; i<cols.count ; i++) {
            ...
            switch (i) {
                case 0:
                    key = kCDTestID;break;
                case 1:
                    key = kCDTestName;break;
                case 2:
                    key = kCDTestDescription;break;
                case 3:
                    key = kCDQuestion;break;
                case 4:
                    key = kCDQuestionOption1;break;
                case 5:
                    key = kCDQuestionOption2;break;
                case 6:
                    key = kCDQuestionOption3;break;
                case 7:
                    key = kCDQuestionOption4;break;
                case 8:
                    key = kCDQuestionCorrectAnswer;break;
                case 9:
                    key = kCDQuestionHint;break;
                case 10:
                    key = kCDQuestionID;break;
                default:
                    break;
            }
        ...
        if(![[dict valueForKey:kCDTestID] isEqualToString:@""])
            [arr addObject:dict];
    }
